...
const code = {
	...
	objectStorage: await processCodeDefined({
		...
		loadRaw: () => import('/src/lib/objectStorage/client/uploadToCloudStorage.ts?raw'),
	}),
	api: {
		client: processCode([
			{
	...
	aws: processCode([
		{
			title: 's3.ts',
			loadRaw: () => import('/src/lib/objectStorage/server/s3.ts?raw'),
		},
		{
			title: 'cloudfront.ts',
			loadRaw: () => import('/src/lib/objectStorage/server/cloudfront.ts?raw'),
		},
		{
			title: 'rekognition.ts',
			loadRaw: () => import('/src/lib/objectStorage/server/rekognition.ts?raw'),
		},
	]),
	keyController: await processCodeDefined({
		title: '$lib/objectStorage/server/s3CloudfrontKeyController.ts',
		loadRaw: () => import('/src/lib/objectStorage/server/s3CloudfrontKeyController.ts?raw'),
	}),
	unsavedUploadsCleaner: await processCodeDefined({
		title: '$lib/objectStorage/server/unsavedUploadsCleaner.ts',
		loadRaw: () => import('/src/lib/objectStorage/server/unsavedUploadsCleaner.ts?raw'),
	}),
