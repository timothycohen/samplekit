...
export const sendEmail: Transports['sendEmail'] = {
	verification: async (a) =>
		send({
			to: a.email,
			dynamicTemplateData: {
	...
	passwordReset: async (a: { email: string; token: string }) =>
		send({
			to: a.email,
			dynamicTemplateData: {
	...
	passwordResetNotFound: async ({ email }: { email: string }) =>
		send({
			to: email,
			dynamicTemplateData: {
				subject: 'Reset SampleKit password',
	...
	noResetWithGoogle: async ({ email }: { email: string }) =>
		send({
			to: email,
			dynamicTemplateData: {
				subject: 'Reset SampleKit password',
	...
	passwordChanged: async ({ email }: { email: string }) =>
		send({
			to: email,
			dynamicTemplateData: {
	...
	changedProviderMethod: async (a: { email: string; newProvider: DB.Provider.Id }) =>
		send({
			to: a.email,
			dynamicTemplateData: {
	...
	MFAUpdate: async (a: { email: string; mfaLabel: string; editKind: 'added' | 'removed' }) =>
		send({
			to: a.email,
			dynamicTemplateData: {
				subject: 'MFA Updated',
	...
	veriToSeshConfEmail: async (a: { email: string; token: string; redirectPath: string }) =>
		send({
			to: a.email,
			dynamicTemplateData: {
	...
	delete: async ({ email }: { email: string }) =>
		send({
			to: email,
			dynamicTemplateData: {
