...
import type { Auth } from '@samplekit/auth/server';
...
function getLimSendUnlimAttemptTable(tokenName: DB.Token.Kind.LimSendUnlimAttempt) {
	switch (tokenName) {
		case 'email_veri':
...
function getLimSendTable(tokenName: DB.Token.Kind.LimSend) {
	switch (tokenName) {
		case 'email_veri':
			return { table: tokens.emailVeri };
		case 'pw_reset':
...
function getLimAttemptTable(tokenName: DB.Token.Kind.LimAttempt) {
	switch (tokenName) {
		case 'setup_authenticator':
